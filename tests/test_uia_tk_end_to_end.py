...
from pytest_uia.domain.locator import Element
...
class StillCatchingUp(Exception):
    """The keys are injected; the accessibility tree has not caught up yet.

    Deliberately not an ElementNotFound: the entry was found. What is late is
    the value the application re-announces once Tk's own message pump has run.
    """


def _once_the_tree_has_caught_up(box: Element, expected: str) -> Element:
    read = box.read_text()
    if read != expected:
        raise StillCatchingUp(f"the entry reads {read!r}, not {expected!r}")
    return box
...
def test_typing_into_an_annotated_tk_entry_lands_in_the_widget_and_reads_back_through_its_value(
    ...
    # Given the app's empty title box, found by the name the application had to
    # give it — an entry carries no words of its own to infer one from
    locator = UiaLocator(resolve_main_window(tk_app.pid))
    title = locator.find(TITLE_TEXTBOX)
    assert title.read_text() == "", (
        "an entry nobody has typed into has to read as empty rather than as "
        "its own name, or the assertion below would already be satisfied"
    )
    ...
    # it, its variable changed, the trace re-announced the new value, and the
    # tree gives it back
    settled = poll(
        lambda: _once_the_tree_has_caught_up(title, A_DRAFT),
        _REACTION_POLICY,
        retry_on=StillCatchingUp,
    )
